...
function recordPublishedBatch(receipt) {
  ...
  var obs = upsertCanonicalInterventionObservations_(plan, applied.interventions);
  writeLog_(
    'INFO',
    plan.siteName,
...
function canonicalProductionUrl_(productionUrl) {
  var raw = String(productionUrl || '').trim();
  if (!/^https?:\/\//i.test(raw)) return '';
  return raw.replace(/\/+$/, '');
}

function canonicalObservationUrl_(pagePath, productionUrl) {
  var path = normalizePublishPagePath_(pagePath, productionUrl);
  var base = canonicalProductionUrl_(productionUrl);
  return base ? base + (path === '/' ? '/' : path) : path;
}

/** Observation 的唯一 URL resolver：absolute receipt 优先，否则由 SiteID 查站点配置。 */
function resolveCanonicalObservationPrimaryUrl_(raw, siteId, siteName, receiptProductionUrl) {
  var value = String(raw || '').trim();
  if (/^https?:\/\//i.test(value)) {
    var absolutePath = publishPathnameFromAbsoluteUrl_(value);
    var absoluteBase = value.match(/^(https?:\/\/[^\/?#]+)/i);
    return absoluteBase ? canonicalObservationUrl_(absolutePath, absoluteBase[1]) : absolutePath;
  }
  var base = canonicalProductionUrl_(receiptProductionUrl);
  if (!base && (siteId || siteName)) {
    var site = findEnabledSiteByNameOrId_(siteName || '', siteId || '');
    base = canonicalProductionUrl_(site && site.propertyUrl);
  }
  return canonicalObservationUrl_(value || '/', base);
}

function validInterventionBaselineMode_(value) {
  return [
    'GSC_ALIGNED', 'NEW_SITE_BASELINE', 'NEW_PAGE_NO_PRE_BASELINE',
    'EXISTING_URL_NO_GSC_TRAFFIC', 'BASELINE_UNKNOWN', 'NO_PREDEPLOY_DATA',
    'WAITING_FOR_GSC'
  ].indexOf(String(value || '').trim()) >= 0;
}

function normalizeObservationDateTime_(value) {
  ...
  if (!isNaN(parsed.getTime()) && !/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2}$/.test(raw)) {
...
function buildInterventionObservation_(opts) {
  opts = opts || {};
  var primaryPath = normalizePublishPagePath_(opts.primaryUrl || '/', opts.productionUrl || '');
  var primaryUrl = resolveCanonicalObservationPrimaryUrl_(opts.primaryUrl || '/', opts.siteId, opts.site, opts.productionUrl);
  var horizon = String(opts.horizon || '').trim();
  var interventionId = String(opts.interventionId || '').trim();
  var status = String(opts.status || INTERVENTION_OBSERVATION_STATUS.WAITING_HORIZON).trim();
  if (status === 'PENDING') status = INTERVENTION_OBSERVATION_STATUS.WAITING_HORIZON;
  return {
    ObservationID: buildInterventionObservationId_(interventionId, primaryPath, horizon),
    InterventionID: interventionId,
    ...
    Horizon: horizon,
    TargetDate: normalizeKeyDate_(opts.targetDate),
    ObservedDataDate: normalizeKeyDate_(opts.observedDataDate),
    Status: status || INTERVENTION_OBSERVATION_STATUS.WAITING_HORIZON,
    BaselineDataDate: normalizeKeyDate_(opts.baselineDataDate),
    ...
    UpdatedAt: normalizeObservationDateTime_(opts.updatedAt),
  ...
  for (var i = 0; i < interventions.length; i++) {
    ...
    for (var h = 0; h < horizons.length; h++) {
      ...
      var observation = buildInterventionObservation_({
        ...
        observedDataDate: '',
        status: INTERVENTION_OBSERVATION_STATUS.WAITING_HORIZON,
        baselineDataDate: iv.baselineDataDate || '',
        productionUrl: iv.publishFields && iv.publishFields.ProductionURL,
      ...
      if (hit) {
        var prev = hit.row || [];
        var prevMap = hit.map || {};
        observation.ObservedDataDate = '';
        observation.Status = INTERVENTION_OBSERVATION_STATUS.WAITING_HORIZON;
        observation.BaselineDataDate = observation.BaselineDataDate || ledgerDateCell_(prev, prevMap, 'BaselineDataDate');
        var previousBaselineMode = ledgerCell_(prev, prevMap, 'BaselineMode');
        if (validInterventionBaselineMode_(previousBaselineMode)) {
          observation.BaselineMode = previousBaselineMode;
        }
        [
          'BaselineClicks7D', 'BaselineImpressions7D', 'BaselineCTR', 'BaselinePosition',
          'BaselineQueryCount7D'
...
/** 唯一 observation persistence contract；legacy receipt 只允许通过此入口。 */
function upsertCanonicalInterventionObservations_(plan, interventions) {
  return syncInterventionObservationsForBatch_(plan, interventions);
}

function loadInterventionObservationContext_() {
  var packed = loadLedgerSheetRows_(SHEET_NAMES.CONTENT_UPDATES);
  var out = {};
  for (var i = 0; i < packed.rows.length; i++) {
    var row = packed.rows[i];
    var id = ledgerCell_(row, packed.map, 'InterventionID');
    if (!id) continue;
    out[id] = {
      site: ledgerCell_(row, packed.map, '站点'),
      siteId: ledgerCell_(row, packed.map, 'SiteID'),
      productionUrl: ledgerCell_(row, packed.map, 'ProductionURL'),
      deployedAt: ledgerCell_(row, packed.map, 'ProductionDeployedAt'),
      baselineDataDate: ledgerDateCell_(row, packed.map, 'BaselineDataDate')
    };
  }
  return out;
}

function observationWindowMetrics_(siteName, pagePath, targetDate) {
  var target = normalizeKeyDate_(targetDate);
  if (!siteName || !target) return { complete: false };
  var start = addDaysStr_(target, -6);
  var sheet = getSpreadsheet_().getSheetByName(SHEET_NAMES.PAGES);
  if (!sheet || sheet.getLastRow() < 2) return { complete: false };
  var rows = sheet.getRange(2, 1, sheet.getLastRow() - 1, PAGE_HEADERS.length).getValues();
  var byDate = {};
  for (var i = 0; i < rows.length; i++) {
    var row = rows[i];
    if (String(row[1] || '').trim() !== siteName) continue;
    if (normalizePublishPagePath_(row[2] || row[3] || '') !== pagePath) continue;
  ...
  var positionCount = 0;
  for (var d = 0; d <= 6; d++) {
    var dateKey = addDaysStr_(start, d);
    var item = byDate[dateKey];
    if (!item) return { complete: false };
    clicks += Number(item[4] || 0);
    impressions += Number(item[5] || 0);
    if (item[7] !== '' && !isNaN(Number(item[7]))) {
      positionTotal += Number(item[7]);
      positionCount++;
    }
  }
  return {
    ...
    position: positionCount ? positionTotal / positionCount : '',
    queryCount: ''
  };
}

function latestDailyDataDateForSite_(siteName) {
  return latestDateInRows_(loadDailyRowsForSiteName_(siteName), 0);
}

function canonicalizeInterventionObservation_(row, packed, context, now) {
  var current = {};
  INTERVENTION_OBSERVATION_HEADERS.forEach(function (header) {
    current[header] = ledgerRawCell_(row, packed.map, header);
  });
  var interventionId = ledgerCell_(row, packed.map, 'InterventionID');
  var ctx = context[interventionId] || {};
  var site = ledgerCell_(row, packed.map, 'Site') || ctx.site;
  var siteId = ledgerCell_(row, packed.map, 'SiteID') || ctx.siteId;
  var productionUrl = ctx.productionUrl || '';
  var rawPrimaryUrl = ledgerCell_(row, packed.map, 'PrimaryURL');
  var path = normalizePublishPagePath_(rawPrimaryUrl, productionUrl);
  var primaryUrl = resolveCanonicalObservationPrimaryUrl_(rawPrimaryUrl, siteId, site, productionUrl);
  var horizon = ledgerCell_(row, packed.map, 'Horizon');
  var target = ledgerDateCell_(row, packed.map, 'TargetDate');
  var baselineDate = ledgerDateCell_(row, packed.map, 'BaselineDataDate') || ctx.baselineDataDate || '';
  var observation = buildInterventionObservation_({
    interventionId: interventionId,
    decisionId: ledgerCell_(row, packed.map, 'DecisionID'),
    siteId: siteId,
    site: site,
    primaryUrl: primaryUrl,
    ...
    horizon: horizon,
    targetDate: target,
    baselineDataDate: baselineDate,
    baselineMode: validInterventionBaselineMode_(ledgerCell_(row, packed.map, 'BaselineMode'))
      ? ledgerCell_(row, packed.map, 'BaselineMode')
      : (baselineDate ? BASELINE_MODE.GSC_ALIGNED : ''),
    attributionMode: ledgerCell_(row, packed.map, 'AttributionMode') || 'OBSERVATIONAL_ONLY',
    status: INTERVENTION_OBSERVATION_STATUS.WAITING_HORIZON,
    updatedAt: now
  });
  [
    'BaselineClicks7D', 'BaselineImpressions7D', 'BaselineCTR', 'BaselinePosition',
    'BaselineQueryCount7D', 'BaselineSiteImpressions7D', 'BaselineSiteClicks7D'
  ].forEach(function (header) {
    var value = ledgerRawCell_(row, packed.map, header);
    if (!ledgerBlank_(value)) observation[header] = value;
  });
  var latest = latestDailyDataDateForSite_(site);
  var metrics = latest && target && latest >= target
    ? observationWindowMetrics_(site, path, target)
    : { complete: false };
  if (!target || !horizon || !interventionId || !site) {
    observation.Status = INTERVENTION_OBSERVATION_STATUS.WAITING_DATA;
  } else if (todayStr_() < target) {
    observation.Status = INTERVENTION_OBSERVATION_STATUS.WAITING_HORIZON;
  } else if (!latest || latest < target) {
    observation.Status = INTERVENTION_OBSERVATION_STATUS.WAITING_DATA;
  } else if (!metrics.complete) {
    observation.Status = INTERVENTION_OBSERVATION_STATUS.WAITING_DATA;
  } else {
    observation.Status = INTERVENTION_OBSERVATION_STATUS.OBSERVED;
    observation.ObservedDataDate = target;
    observation.ObservedClicks7D = metrics.clicks;
    observation.ObservedImpressions7D = metrics.impressions;
    observation.ObservedCTR = metrics.ctr;
    observation.ObservedPosition = metrics.position;
    observation.ObservedQueryCount7D = metrics.queryCount;
  }
  if (observation.Status === INTERVENTION_OBSERVATION_STATUS.OBSERVED) {
    var baseline = observationWindowMetrics_(site, path, baselineDate);
    if (baseline.complete) {
      observation.BaselineClicks7D = baseline.clicks;
      observation.BaselineImpressions7D = baseline.impressions;
      observation.BaselineCTR = baseline.ctr;
      observation.BaselinePosition = baseline.position;
      observation.BaselineQueryCount7D = baseline.queryCount;
      observation.ClicksDelta = observation.ObservedClicks7D - baseline.clicks;
      observation.ImpressionsDelta = observation.ObservedImpressions7D - baseline.impressions;
      observation.CTRDelta = observation.ObservedCTR - baseline.ctr;
      observation.PositionImprovement = baseline.position === '' || observation.ObservedPosition === ''
        ? '' : baseline.position - observation.ObservedPosition;
      observation.QueryCountDelta = '';
    }
  }
  observation.ObservationID = buildInterventionObservationId_(interventionId, path, horizon);
  return observation;
}

function reconcileInterventionObservations_() {
  ensureLedgerHeader_(SHEET_NAMES.INTERVENTION_OBSERVATIONS, INTERVENTION_OBSERVATION_HEADERS);
  var packed = loadLedgerSheetRows_(SHEET_NAMES.INTERVENTION_OBSERVATIONS);
  var context = loadInterventionObservationContext_();
  var now = nowRecordedAt_();
  var seen = {};
  var updated = 0;
  for (var i = packed.rows.length - 1; i >= 0; i--) {
    var rowIndex = i + 2;
    var observation = canonicalizeInterventionObservation_(packed.rows[i], packed, context, now);
    var key = observation.ObservationID;
    if (key && seen[key]) {
      packed.sheet.deleteRow(rowIndex);
      continue;
    }
    if (key) seen[key] = true;
    packed.sheet.getRange(rowIndex, 1, 1, packed.header.length)
      .setValues([observationRowFromObject_(observation, packed, null)]);
    updated++;
  }
  return { updated: updated };
}

/** 显式线上 repair 入口；只按实际 header 读写，重复执行幂等。 */
function repairInterventionObservationRows() {
  return reconcileInterventionObservations_();
}

/** 一次性 targeted backfill：只恢复有明确 SITE_LAUNCH durable evidence 的 BaselineMode。 */
function backfillMissingLaunchBaselineMode() {
  ensureLedgerHeader_(SHEET_NAMES.INTERVENTION_OBSERVATIONS, INTERVENTION_OBSERVATION_HEADERS);
  ...
  var launchKeys = {};
  for (var c = 0; c < content.rows.length; c++) {
    var contentRow = content.rows[c];
    var action = ledgerCell_(contentRow, content.map, 'Action');
    var legacyAction = ledgerCell_(contentRow, content.map, '更新类型');
    if (String(action || '').trim().toUpperCase() !== 'SITE_LAUNCH' &&
        String(legacyAction || '').trim().toUpperCase() !== 'SITE_LAUNCH') continue;
    ['BatchID', 'InterventionID', 'PageReceiptKey', 'DecisionID', 'SiteID', 'Action'].forEach(function (header) {
      var key = ledgerCell_(contentRow, content.map, header);
      if (key) launchKeys[key] = true;
    });
  }
  var modeIndex = observations.map.BaselineMode;
  if (modeIndex === undefined) throw new Error('backfillMissingLaunchBaselineMode: BaselineMode header missing');
  var result = { scanned: 0, repaired: 0, unchanged: 0, skipped: 0, errors: [] };
  for (var i = 0; i < observations.rows.length; i++) {
    result.scanned++;
    var row = observations.rows[i];
    var currentMode = ledgerCell_(row, observations.map, 'BaselineMode');
    if (currentMode) {
      result.unchanged++;
      continue;
    }
    var interventionId = ledgerCell_(row, observations.map, 'InterventionID');
    var decisionId = ledgerCell_(row, observations.map, 'DecisionID');
    if (!launchKeys[interventionId] && !launchKeys[decisionId]) {
      result.skipped++;
      continue;
    }
    try {
      observations.sheet.getRange(i + 2, modeIndex + 1).setValue('NEW_SITE_BASELINE');
      result.repaired++;
    } catch (e) {
      result.errors.push('row ' + (i + 2) + ': ' + String(e.message || e));
    }
  }
  return result;
}
...
function repairPublishedBatchLedger_(batchId, receipt) {
  ...
  var obs = upsertCanonicalInterventionObservations_({
    siteId: batchSiteId,
